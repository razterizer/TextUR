...
class Game : public t8x::GameEngine<44, 92, CharT>
{
  ...
  void show_help() const
  {
    ...
    std::cout << "   [--edit_textures_as_ascii_only]" << std::endl;
  ...
  void reset_textel_editor(bool init = false)
  {
    ...
    if (init)
    {
      ...
      if (edit_textures_as_ascii_only)
      {
...
public:
    ...
    , message_handler(std::make_unique<t8x::MessageHandler<std::string>>())
  {
    ...
    for (int a_idx = 1; a_idx < argc; ++a_idx)
    {
      ...
      else if (std::strcmp(argv[a_idx], "--edit_textures_as_ascii_only") == 0)
        edit_textures_as_ascii_only = true;
...
//#define SHOW_DEBUG_WINDOW
        ...
        switch (edit_mode)
        {
          ...
          case EditTextelMode::EditTextelNormal:
          {
            ...
            if (curr_special_key == t8::SpecialKey::Enter)
            {
              ...
              else if ((edit_textures_as_ascii_only && dialog_editor.text_field_empty(1))
                    || (!edit_textures_as_ascii_only && dialog_editor.glyph_picker_empty(1)))
              {
                message_handler->add_message(static_cast<float>(get_real_time_s()),
                                             "You must enter a textel " + (edit_textures_as_ascii_only ? "character"s : "glyph"s) + ".",
                                             t8x::MessageHandlerLevel::Guide);
              }
          ...
          case EditTextelMode::EditTextelShadow:
          {
            ...
            if (curr_special_key == t8::SpecialKey::Enter)
            {
              ...
              else
              {
                ...
                {
                  if (!edit_textures_as_ascii_only && gp_textel_symbol != nullptr)
  ...
  bool edit_textures_as_ascii_only = false;
